package main.manager.output.conf;
...
public class Kafka {
    ...
    public static final String MESSAGE_GET_DEVICES = "messageGetDevices";
    ...
    @Bean(MESSAGE_GET_DEVICES)
    public KafkaTemplate<String, MessageGetDevices> messageGetDevices() {
        return new KafkaTemplate<>(producerFactory(
                props -> {
                    props.put(ProducerConfig.ACKS_CONFIG, "all");
                })
    ...
    public KafkaTemplate<String, NewDevice> newDevice() {
        ProducerFactory<String, NewDevice> tmp = producerFactory(
                props -> {
                    props.put(ProducerConfig.ACKS_CONFIG, "all");
                }
        );
        return new KafkaTemplate<>(tmp);
    }
    ...
    @Bean
    public ConcurrentKafkaListenerContainerFactory<String, MessageAuth> getMessageAuth(){
    ...
    @Bean
    public ConcurrentKafkaListenerContainerFactory<String, MessageGetDevices> getMessageGetDevices(){
        var props = properties.buildConsumerProperties(null);
        props.put(ConsumerConfig.ENABLE_AUTO_COMMIT_CONFIG, "true");
        props.put(ConsumerConfig.ISOLATION_LEVEL_CONFIG, "read_committed");
        ConcurrentKafkaListenerContainerFactory<String, MessageGetDevices>
                factory = new ConcurrentKafkaListenerContainerFactory<>();
        factory.setConsumerFactory(
                new DefaultKafkaConsumerFactory<>(
                        props,
                        new StringDeserializer(),
                        new JsonDeserializer<>(MessageGetDevices.class)
                )
        );
        return factory;
    }
    @Bean
    public ConcurrentKafkaListenerContainerFactory<String, NewDevice> getNewDevice(){
        var props = properties.buildConsumerProperties(null);
        props.put(ConsumerConfig.ENABLE_AUTO_COMMIT_CONFIG, "true");
        props.put(ConsumerConfig.ISOLATION_LEVEL_CONFIG, "read_committed");
