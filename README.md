...
| Компонент |Описание                        | Сложность |Размер|
|-----------|--------------------------------|--|--|
|Verifier|Выполняет аутентификацию критических устройств, проверяет подпись переданных данных| M | L |
...
|MainManagerOutput|Отправляет данные в облако после аутентификации через Verifier| S | L |
|ClientAuth|Аутентификация клиентов| M | L |
|ClientStorage|Хранит данные клиентов|C|XL|
|ManagerInput|Получает запрос от клиентов и передаёт их в ManagerOutput и MainManagerOutput| S | L |
|Hub|Получает данные от девайсов и передаёт в Storage| S | S |
